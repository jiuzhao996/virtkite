...
| 字段 | 说明 |
|---|---|
| name、libvirt_uri、ssh_ip、ssh_port、ssh_user | 连接信息。`ssh_ip` 写入前须过格式校验（IP 或保守主机名白名单）——该值会作为 argv 传给 `ping`，以连字符开头的取值会被当成命令选项，见 docs/05 §3 |
| status、cpu/mem/disk、os_version、description | 状态采集与描述。`status` 取 `unknown / reachable / unreachable`，由连通性测试回写 |
