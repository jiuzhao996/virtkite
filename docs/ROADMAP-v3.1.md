# 鸢航 VirtKite v3.1 施工级规划（除 O 批未实施外，其余批次全部落地）

> **状态（2026-09-21 更正）**：批次 0 + M1 + A/B/C/D/E/G/I/K/L/N/P + M3/M4 全部完成，本地提交至 `17911f6`，**未推送**。
> **O 批（VM 标签/分组）未实施**——`model/vm.go` 无 tags/group 字段，全仓库无落地（此前状态行误将 O 列入完成清单，现予除名；VM 分组/标签列为后续展望）。原规划中「K 批拓扑依赖 O」的说法一并更正：拓扑页已上线，按**状态**着色，不依赖分组字段。
> 平台运行中；验收入口见各批次 DoD；回退锚点 `47276ad`（v3 前）或 `aced919`（v2 完成态）。
> 未做（刻意）：M2 PageShell、M5 页内导航组件化、F 概览改版（纯 UI 打磨，记录在案视后续）；**O 批 VM 标签/分组未实施（除名，见头部状态行）**。
...
| E | 工具箱 | 功能 | 1.5h | 无 |
| N | 回收站（新） | 功能 | 2h | 无 |
| O | VM 标签/分组（新） | 功能 | 2h | 无（**未实施，除名**） |
| K | 拓扑可视化 | 功能 | 1.5h | 原规划可选依赖 O；实际按状态着色落地，不依赖分组字段 |
| H | 云镜像市场 | 功能 | 2h | 无 |
| J | VM 导出/导入 | 功能 | 2h | 无 |
...
O（VM 标签）──（未实施）──✕ K（拓扑图实际按状态着色，不依赖 O）
其余批次互相独立，可任意穿插
```
...
| **v3.2 打磨包** | N + E + K + H + J + L + F/M2/M4/M5（O 批未实施） | 视余量裁剪，完成一项验一项 |
...
### O VM 标签/分组（新）——**未实施（除名）**
- 原设计要点：vms 加 `tags` JSON 数组列；列表页标签筛选 + 颜色可配；拓扑图按标签着色
- 原定 DoD：打标/筛选/按组统计（仪表盘加分组分布）
- **现状（2026-09-21 核实）**：本批从未实施——`model/vm.go` 无 tags/group 字段，全仓库无任何落地代码；拓扑图按 VM **状态**着色（已上线），「按标签着色」不存在。记录在案，VM 分组/标签列后续展望，勿在任何文档宣称已有此功能。
