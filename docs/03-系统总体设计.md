...
flowchart TB
    ...
    RBAC["权限中间件<br/>AdminMiddleware / OperatorMiddleware / NonViewerMiddleware"]
...
| 存储层 | MySQL 8（`database.Init()` AutoMigrate 11 张表，连接池显式配置） | `users / hosts / vms / images / audit_logs / tasks / console_sessions / system_settings / alerts / pool_meta / vm_grants` |
...
路由与中间件装配（`main.go`）：公开 `POST /api/auth/login` 与 `GET /api/vnc/token/:token`（供 websockify 内网解析）；`/api` 组统一经过 `AuthMiddleware`，各资源组按敏感度挂 `AdminMiddleware`（`/users`、`/audit`、`/settings`）、`OperatorMiddleware`（`/hosts`、`/vms`、`/storage`、`/networks`、`/images`、`/dashboard`、`/tasks`、`/sessions`）或 `NonViewerMiddleware`（`/monitor`——告警与 file-sd 携带资产名单，viewer 403）。operator 的写操作被收敛在 `/api/vms` 前缀内（虚拟机全生命周期，含三类控制台），基础设施模块只读。授权管理三端点（`/api/vms/:id/grants`）虽在 `/vms` 组内，handler 内以 `requireAdminRole` 二次收口仅 admin 可用；非 admin 对 VM 的读访问经 `vmVisible`/`grantedVMIDs` 按 `vm_grants` 未过期授权过滤，未授权返回 404。单例装配：`taskMgr := tasks.NewManager(db)` + `tasks.RegisterVMTasks(taskMgr)`，`consoleRegistry := console.NewRegistry(db)` + `StartSweeper()`，注入各 Handler。
