...
- 单后端二进制 + 前端静态产物。产物目录按四候选依次探测：`<可执行文件目录>/web/dist` → `<可执行文件目录>/static` → `<当前工作目录>/web/dist` → `<当前工作目录>/static`，命中即记入启动日志；全部未命中时降级为「仅 API」模式并对页面请求返回 503 中文提示，不终止进程。
...
- 生产模式（`SERVER_MODE=release`）启动前必须显式设置 `JWT_SECRET_KEY`，且不得等于内置默认值，否则进程拒绝启动。
