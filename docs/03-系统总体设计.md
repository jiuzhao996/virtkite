...
| SSH 跳板（v3.6） | 内置 SSH 服务端（:2222）：密码认证（users 表 bcrypt + per-IP 限流）→ 资产菜单（复用 vm_grants）→ 连接前重验 → vmssh 桥接直达 VM shell；会话落 console_sessions（type=jump） | `service/jumpd`（limiter/menu/auth/hostkey/server 五单元）、`config.JumpdEnabled/JumpdPort`、`main.go` 启动接线 |
