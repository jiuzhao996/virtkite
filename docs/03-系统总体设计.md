...
| 异步任务子系统 | 任务队列 + worker 池；创建/删除/克隆/优雅关机/孤儿卷清理后台化；真分页；`202 + {task_id}` + 前端轮询 | `service/tasks/manager.go`（`Manager/NewManager/Submit/Get/List/ListPaged/Register`）、`service/tasks/vm_tasks.go`（`RegisterVMTasks` + 6 executors，含 cleanup_volumes 孤儿卷清理）、`handler/task.go`（`TaskHandler/ListTasks/GetTask/DeleteTask`）、`web/src/utils/task.js`（`pollTask`）、`web/src/views/TaskList.vue` |
...
| 监控中心 | Alertmanager 告警代理、Grafana kiosk 看板嵌入、历史性能曲线（query_range 回放） | `handler/monitor.go`、`handler/history.go`、`web/src/views/Monitor.vue` |
| 网络管理 | 网络列表/详情、启停/删除、NAT 创建、XML 编辑、自启动开关 | `service/virt/network.go`（`SetNetworkAutostart`）、`handler/network.go` |
...
| 系统设置 | 可写运行参数（system_settings KV 表，保存即生效）+ 生效配置快照 | `model/setting.go`、`service/setting/setting.go`（白名单校验+缓存）、`handler/settings.go`（`GetSettings/UpdateSettings`）、`web/src/views/Settings.vue` |
...
`/metrics` 为满足 Prometheus 抓取而公开无鉴权；登录接口无失败次数限流；CORS 默认允许任意来源；
Web 终端为适配 IP 动态分配的短生命周期虚拟机而跳过 SSH 主机密钥校验。
...
选择的取舍**——一个垃圾文件可事后清理，一块损坏的在用磁盘不可恢复。孤儿卷的清理入口列入后续工作
（docs/08 §2.3）。
