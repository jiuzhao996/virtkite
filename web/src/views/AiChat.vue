            ...
            <div class="bubble" :class="{ 'is-error': m.error && !m.content }">
              <template v-if="m.content">
                <template v-for="(seg, si) in splitSegments(m.content)" :key="si">
                  <pre v-if="seg.code" class="msg-code"><code>{{ seg.text }}</code></pre>
                  <span v-else class="msg-text">{{ seg.text }}</span>
                </template>
              </template>
...
import { ChatDotRound, Delete, MagicStick, Promotion, Refresh, User, Warning } from '@element-plus/icons-vue'
...
async function streamAnswer() {
  // 组装历史：只送有效内容（错误气泡与空占位不进上下文）
  const history = messages.value
    .filter((m) => !m.error && m.content)
    .map((m) => ({ role: m.role, content: m.content }))
  const assistant = reactive({ role: 'assistant', content: '', streaming: true, error: '' })
...
/**
 * 按 ``` 分割消息内容为 文本/代码 段序列（不引 markdown 库）。
 * 偶数下标是普通文本，奇数下标是代码块；代码块首行是语言标记（如 bash）时丢弃。
 * 流式中代码块刚开栅还没收到换行时整段可能只是语言标记，先不渲染，下个 delta 到达后整体重渲染。
 */
function splitSegments(content) {
  ...
  String(content || '')
    ...
    .forEach((seg, i) => {
      if (i % 2 !== 1) {
        if (seg) segs.push({ code: false, text: seg })
        return
      }
      const nl = seg.indexOf('\n')
      if (nl < 0) return
      const first = seg.slice(0, nl).trim()
      let text = first && !/\s/.test(first) ? seg.slice(nl + 1) : seg
      text = text.replace(/^\n/, '').replace(/\n$/, '')
      if (text) segs.push({ code: true, text })
    })
  return segs
}
...
/* 代码块：深色底等宽字体，配色取自 ConsolePage 终端面板（#0e1626 / #e6edf3） */
.msg-code {
  margin: 6px 0;
  padding: 10px 14px;
