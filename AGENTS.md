...
- **⚠️ SEED_DIR 是本机路径（部署时必须改）**：本机 `.env` 已把 `SEED_DIR` 统一到 `/home/jiuzhao/storage/seed`（与旧脚本目录合并）。**部署到其它机器时必须改为该宿主机的可写目录**（如 `/var/lib/vmops/seed`），勿照抄本机路径——该目录需 web 进程可写、qemu 进程可读。
- **✅ 公网入口已上线（2026-09）：https://kpyun.fun**——架构=腾讯云(152.136.110.39, 4C4G, 无嵌套虚拟化不能跑KVM) nginx TLS(证书 /etc/nginx/kpyun/, vhost conf.d/kpyun.conf, 归档在 deploy/kpyun/) → 隧道已切换为 frp（2026-09-11，安全组已放行 7000）：云端 frps.service + 本机 virtkite-frpc.service（/usr/local/frp/，proxyBindAddr=127.0.0.1 防绕过 TLS），旧 autossh 服务 virtkite-tunnel 已 disable 备用 → 家里 vmops/websockify。**腾讯云标准 CVM 无法开启嵌套虚拟化**（无 vmx 透传，产品策略非软件开关），云机当 KVM 宿主机此路不通；frp 配置与切换记录见 deploy/kpyun/frp-方案.md。腾讯云侧加固（2026-09-11）：sshd 仅密钥登录（Ubuntu 注意 50-cloud-init.conf 与主配置两处 PasswordAuthentication 需同时清，sshd 首次出现优先）、tinyproxy 已停用、VictoriaMetrics 只绑回环（公网经 nginx 8081 TLS+BasicAuth）。公网暴露已加固：.env SERVER_MODE=release + 强随机 JWT_SECRET_KEY + CORS_ORIGINS=kpyun.fun + METRICS_TOKEN/ALERT_WEBHOOK_TOKEN 双令牌（Prometheus 抓取 bearer_token、AM webhook 带 token，deploy/ 配置联动）；ConsolePage VNC URL 在 HTTPS 下走同源 /vnc/ 前缀（nginx wss 升级），本地 http 直连 6080 行为不变。
- **⚠️ 存储池布局约定（2026-09 规划，文件已定址勿搬迁）**：`base`=模板基盘（backing 父盘与模板，Ubuntu.img/rocky10.qcow2/Rocky.img 均被引用）、`images`=增量系统盘、`exten`=数据盘、`img`=安装镜像（ISO）、`default`=系统池（/var/lib 权限高不日常用）。**域 XML 与 backing file 记绝对路径，任何被引用卷移动 = VM 报废**；整理语义只能走平台侧（pool_meta/徽标），不能搬文件。`vmops` 池已删除，默认池（setting `default_storage_pool`，fallback 常量同步改）为 `images`。
