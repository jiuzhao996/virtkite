...
- **✅ 公网入口已上线（2026-09）：https://kpyun.fun**——架构=腾讯云(152.136.110.39, 4C4G, 无嵌套虚拟化不能跑KVM) nginx TLS(证书 /etc/nginx/kpyun/, vhost conf.d/kpyun.conf, 归档在 deploy/kpyun/) → SSH 反向隧道(本机 systemd 服务 `virtkite-tunnel`：autossh -R，云 18080/16081 → 家 8080/6080) → 家里 vmops/websockify。**腾讯云标准 CVM 无法开启嵌套虚拟化**（无 vmx 透传，产品策略非软件开关），云机当 KVM 宿主机此路不通；frp 备选方案见 deploy/kpyun/frp-备选方案.md（需控制台放行 7000）。公网暴露已加固：.env SERVER_MODE=release + 强随机 JWT_SECRET_KEY + CORS_ORIGINS=kpyun.fun；ConsolePage VNC URL 在 HTTPS 下走同源 /vnc/ 前缀（nginx wss 升级），本地 http 直连 6080 行为不变。
